...
void *PrintString_Threads(void *thread_params)
{
    struct thread_params *params;//обьявление структуры

    params= (struct thread_params *) thread_params;
    printf("%s \n",params->char_text);
    printf("%d \n",params->int_text);
...
int main()
{
    pthread_t tred;//создание потока
    struct thread_params params;//создание структуры
    params.char_text="AAAAAA!";//заполение структуры
    params.int_text=12345;//заполнение структуры
    pthread_create(&tred, NULL, PrintString_Threads,(void *)&params);//создание и запуск потока/ последним параметром передается структура
